...
def check_admin():
    ...
    if is_admin:
        # Verify that the current logged-in user matches the admin session
        if session.get('user') != session.get('admin_username'):
            session.pop('admin_logged_in', None)
            session.pop('admin_username', None)
            session.pop('is_default_admin', None)
            return jsonify({'logged_in': False})
...
def admin():
    ...
    if request.method == "POST":
        ...
        # Verify the username matches the current logged-in user
